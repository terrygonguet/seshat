class Project {
  ...
  createSession(start, duration) {
    let end = start.clone().add(duration);
    let events = [
      Session.Event.Type.started + " " + start.toISOString(),
      Session.Event.Type.finished + " " + end.toISOString()
    ];
    this.sessions.push(new Session({ events }));
    this.sessions.sort((a,b)=>(a.startedAt.isBefore(b.startedAt) ? -1 : 1));
  }
